...
List lda_pcgs_v2(
  ...
  arma::vec log_posterior_z;
  arma::mat theta_samples;
  arma::mat beta_samples;
  
  unsigned int num_theta_samples, num_beta_samples; 
  if (save_theta) {
    num_theta_samples = theta_indices.n_cols;
    theta_samples = arma::zeros<arma::mat>(num_theta_samples, valid_samples);
  }
  if (save_beta) {
    num_beta_samples = beta_indices.n_cols;
    beta_samples = arma::zeros<arma::mat>(num_beta_samples, valid_samples);
  }
  if (save_lp) { 
    log_posterior = arma::zeros<arma::vec>(valid_samples); 
    log_posterior_z = arma::zeros<arma::vec>(valid_samples); 
  }
  if (test_set_share > 0){
  ...
  cout << endl << endl;
  if (verbose > 1){
    cout << "lda_pcgs (c++): Number of saved samples - "
         << valid_samples << endl;
  }
  
  
  if (verbose > 0){
    cout << "lda_pcgs (c++): Initializes variables and count statistics....";
  }
  ...
  if (save_z) {
    Z_samples = arma::zeros<arma::umat>(num_words, valid_samples); // n x valid_samples matrix
  } else {
    Z_samples = arma::zeros<arma::umat>(num_words, 1); // n x 1 matrix (vector)
  }
  arma::uvec Z = arma::zeros<arma::uvec>(num_words);
  ...
  if (verbose > 1){
    cout << "lda_pcgs (c++): Number of docs: " << num_docs << endl;
    cout << "lda_pcgs (c++): Number of total words: " << num_words << endl;
    cout << "lda_pcgs (c++): Number of test words: " << num_test_words << endl;
    cout << "lda_pcgs (c++): Number of topics: " << num_topics << endl;
    cout << "lda_pcgs (c++): Vocabulary size: " << vocab_size << endl;
    cout << "lda_pcgs (c++): alpha_h: " << alpha_h << endl;
    cout << "lda_pcgs (c++): eta_h: " << eta_h << endl;
  }
  ...
  if (verbose > 0){
    cout << "lda_pcgs (c++): Partially-Collapsed Gibbs sampling..." << endl;
  }
  ...
  for (iter = 0; iter < max_iter; iter++) { // for each Gibbs iteration
    ...
    if (verbose > 1) { cout << "lda_pcgs (c++): gibbs iter# " << iter + 1; }
    ...
    if (save_flag && save_beta) {
      for (k = 0; k < num_beta_samples; k++)
        beta_samples(k, ss_idx) = beta_t(beta_indices(0, k), beta_indices(1, k));
    }
    ...
    for (d = 0; d < num_docs; d++) { // for each document
      ...
      // samples \theta
      if (save_flag && (save_theta || save_lp || ((iter + 1) == max_iter))){
      ...
      for (i = 0; i < n_d; i++) { // for each word
        ...
        // decrements the counts by one, to ignore the contributions of 
        // the current word
        theta_counts(topic_id, d) -= 1.;
        beta_counts(topic_id, word_id) -= 1.;
        corpus_topic_counts(topic_id) -= 1.;
        ...
        new_topic_id = sample_multinomial((theta_counts.col(d) + alpha_h) % beta_t.col(word_id)); 
    ...
    if (save_flag && save_theta) {
      for (k = 0; k < num_theta_samples; k++) 
        theta_samples(k, ss_idx) = theta_t(theta_indices(1, k), theta_indices(0, k));
    }
    ...
    if (save_flag && save_z) { 
      ...
      Z_samples.col(ss_idx) = Z;
    }
    else if (save_flag && ((iter + 1) == max_iter)) { 
      // the last sample is saved always 
      Z_samples.col(0) = Z;
    }
    ...
    if (save_flag && save_lp){
      ...
      log_posterior_z(ss_idx) = logp;
      if (verbose > 1){ cout << " lp-z: " << logp; }
    }
  ...
  if (verbose > 0){
    cout << endl;
    cout << "lda_pcgs (c++): Completed sampling." << endl;
  }
  ...
  return List::create(Named("corpus_topic_counts") = wrap(corpus_topic_counts),
                      ...
                      Named("log_posterior_z") = wrap(log_posterior_z),
