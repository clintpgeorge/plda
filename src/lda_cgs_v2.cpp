...
List lda_cgs_v2(
  ...
  cout << endl << endl;
  if (verbose > 1){
    cout << "lda_cgs (c++): Number of saved samples - "
         << valid_samples << endl;
  }
  
  
  if (verbose > 0){
    cout << "lda_cgs (c++): Initializes variables and count statistics....";
  }
  ...
  for (d = 0; d < num_docs; d++){
    ...
    if (test_set_share > 0){
      unsigned int n_d = doc_word_counts(d); // the document length
  ...
  if (verbose > 1){
    cout << "lda_cgs (c++): Number of docs: " << num_docs << endl;
    cout << "lda_cgs (c++): Number of total words: " << num_words << endl;
    cout << "lda_cgs (c++): Number of test words: " << num_test_words << endl;
    cout << "lda_cgs (c++): Number of topics: " << num_topics << endl;
    cout << "lda_cgs (c++): Vocabulary size: " << vocab_size << endl;
    cout << "lda_cgs (c++): alpha_h: " << alpha_h << endl;
    cout << "lda_cgs (c++): eta_h: " << eta_h << endl;
  }
  ...
  if (verbose > 0){
    cout << "lda_cgs (c++): Collapsed Gibbs sampling..." << endl;
  }
  ...
  for (iter = 0; iter < max_iter; iter++) { // for each Gibbs iteration
    ...
    if (verbose > 1) { cout << "lda_cgs (c++): gibbs iter# " << iter + 1; }
    ...
    if (save_flag && (save_beta || ((iter + 1) == max_iter) || save_lp)) {
  ...
  if (verbose > 0){
    cout << endl;
    cout << "lda_cgs (c++): Completed sampling." << endl;
  }
